...
- Keep consultation as a homepage entry only; do not show form fields on the homepage. Clicking the hero CTA navigates to an in-app consultation page (same pattern as guides/articles).
- Use lightweight hash routes so links are ready to click/share: `#/`, `#/consult`, `#/guide/<id>`, `#/article/<slug>`. Prefer real `<a href>` for guides, articles, consultation and official school sites.
...
- Home rows open in-app guide/article views: study guide covers private/skills school shortlists + the eight public universities + application timeline; career guide covers Seek/Trade Me job advice. Guide pages group articles by topic; articles use rich blocks (headings, tables, steps, figures, tips) with clickable official-site buttons.
- Study content has two tracks. (1) Private/skills: AIS, ICL, Yoobee, NZSE/NZSEG (NESEG is usually a misspelling of NZSEG), and Future Skills-style providers with concrete NZD tuition ranges—one comparison overview plus deep dives (`school-ais`, `school-icl`, `school-yoobee`, `school-nzse`, `school-future-skills`). Be practical about NZQA level, PTE/IELTS/NZCEL pathways, Auckland rent, and “verify fees on official sites.” (2) Eight public universities: one overview (`nz-eight-universities`) plus a dedicated article per university (`uni-auckland`, `uni-otago`, `uni-massey`, `uni-victoria`, `uni-canterbury`, `uni-waikato`, `uni-lincoln`, `uni-aut`) with official `.ac.nz` website links and approximate QS World University Rankings (label year; ranks change; verify on QS and university sites). Do not let rankings replace programme fit, city/budget, or private-school paths. Life guide is a third homepage row; featured life article is the monthly living-cost budget (`nz-monthly-budget`) with approximate 2025–2026 NZD ranges (Auckland shared low/mid totals, cheaper cities noted, CNY rough note)—label figures as approximate and verify locally.
